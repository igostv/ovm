...
Процедура ВыполнитьМиграцию() Экспорт
	...
	Если ЭтоWindows Тогда
		...
		Если СтрНайти(ПутьКДефолтномуOneScript, "ovm") > 0 Тогда
			Лог.Информация("OneScript уже под контролем ovm");
			Возврат;
		КонецЕсли;

		ПеременнаяOVM_OSCRIPTBIN = ПолучитьПеременнуюСреды("OVM_OSCRIPTBIN", РасположениеПеременнойСреды.Пользователь);
		Если ПустаяСтрока(ПеременнаяOVM_OSCRIPTBIN) Тогда
			ВызватьИсключение "Не установлено значение переменной OVM_OSCRIPTBIN на уровне пользователя.
							  |Выполните активацию OneScript";
		КонецЕсли;
		
		Лог.Отладка("Установка переменных среды на уровне системы");
		
		ПеременнаяOVMвPATH = "%OVM_OSCRIPTBIN%;";
		ПеременнаяPATH = ПолучитьПеременнуюСредыИзРеестра("PATH", РасположениеПеременнойСреды.Машина);
		НоваяПеременнаяPATH = ПеременнаяOVMвPATH + СтрЗаменить(ПеременнаяPATH, ПеременнаяOVMвPATH, "");
		УстановитьПеременнуюСредыВРеестре("PATH", "REG_EXPAND_SZ", НоваяПеременнаяPATH,
			РасположениеПеременнойСреды.Машина);
		
		УстановитьПеременнуюСредыСОповещением("OVM_OSCRIPTBIN", ПеременнаяOVM_OSCRIPTBIN,
			РасположениеПеременнойСреды.Машина);
